...
record, as are the `arbitrator`/`arbitratorExtraData` slots behind step 3c, with their own
record (each step's LOGIC is frozen; the constants freeze with their records). The
...
| `arbitrator` | `[ARBITRATOR_ADDRESS]` — **MANDATORY** pin of the registry's arbitrator, proven at every full verification and every fresh point check (§6 step 3c, §8) against storage slot 0. A governor may switch the arbitrator; to a consumer that is a policy-grade change (every later verdict comes from another court), so a mismatch fails closed with the observed address, and a new arbitrator is accepted only through a new signed profile release, never silently. (Owner decision, 2026-09-04; the planned Intendment adoption is exactly such a release.) |
| `arbitratorExtraData` | `[ARBITRATOR_EXTRA_DATA]` — **MANDATORY** pin of the arbitrator's extra data (for Kleros: court id and juror count), proven byte for byte against the Solidity `bytes` at storage slot 1 (§5). A changed court or juror count fails closed the same way. |
| `governor` | `[TIMELOCK_ADDRESS]` — **MANDATORY** pin of the registry's governor, the timelock in front of the Safe (RFC 0001 §9), proven at every full verification and every fresh point check against storage slot 3 (§5, §6 step 3d, §8). The governor is who may queue every other change, so it is part of the trust context; a changed governor fails closed until a new signed profile release names it. |
| `registrationMetaEvidence` / `clearingMetaEvidence` | **MANDATORY** pins of the deployment MetaEvidence/policy references, policy version 0. Trust model: the profile is a locally authenticated release/deployment manifest that pins the factory deployment receipt/events, the registry address, its runtime codehash, and the TWO INITIAL MetaEvidence references those deployment events declared; §6 step 3b's proof of the `metaEvidenceUpdates` counter then establishes WHICH accepted version the registry is at — so the references for that version ARE the policy every later verdict was judged under. Values must be nonempty canonical references; the production checklist (§11) fills them from the deployment receipt |
| `policyVersions` | OPTIONAL list of the policy versions the profile author accepts beyond the deployment one (owner decision 2026-09-06, superseding the immutable-policy rule): each entry names the `metaEvidenceUpdates` counter value that announced it and the two MetaEvidence references that announcement declared, strictly increasing. The accepted set is 0 plus these values; §6 step 3b requires the proven counter to be in it. A governor's policy change, queued publicly through the timelock, is therefore accepted only through a new signed profile release, never silently; every request keeps the policy version it was submitted under (Classic GTCR stores the MetaEvidence id per request); a removal request opened after a change is judged under the version then in force, and an entry is protected only from runtime or duplicate classification rules introduced after the version of the request that established its current registration (listing policy 2.3, Amendments). Because the accepted set is part of the deployment context id, a profile that adds a version rebinds nothing by itself: a client MUST carry existing lockfile entries across only through an explicit migration that requires the new profile to be a strict successor of the old (every other trust field identical, the old accepted versions a prefix of the new), obtains a fresh point check per entry under the new profile plus a local integrity pass, preserves sticky suspensions, and records the transition on the entry (`intend migrate`); the catalog is separate, rebuilt under the new profile with `update`; and the deployment-era profile remains published as a release asset so the migration's `--from` is always available. |
...
- `proofs.slots` MUST cover: the `itemList` length slot; the `metaEvidenceUpdates` slot (§6 step 3b); the `arbitrator` slot and the `arbitratorExtraData` words the pinned length occupies (§6 step 3c); the array slot of every index in `rows`; and the status slot of every item in `rows`.
...
- `governor`: storage slot `3` — PROVISIONAL like slots 0 and 1, pinned by the same fork probe (`cli/tools/probe-arbitrator-slots.sh`, assertion F: slot 3 holds `governor()`, a governor `changeGovernor` moves exactly that word, and the new governor can move it back).
- `arbitrator`: storage slot `0`; `arbitratorExtraData`: storage slot `1`, a Solidity `bytes` — short form (length < 32) inline in the word with `length × 2` in its low byte, long form (length ≥ 32) with `length × 2 + 1` in the word and the data at `keccak256(uint256(1)) + i`. **PROVISIONAL pending review of their evidence record** (`docs/spikes/arbitrator-slot-evidence.md`: an asserting fork probe, `cli/tools/probe-arbitrator-slots.sh`, reads both slots against the getters on a freshly factory-deployed registry of the pinned codehash, then has the governor call `changeArbitrator` with a short and a long extra data and asserts that exactly slots 0, 1 and the `keccak256(1) + i` words move and decode to the getters' values). Verifiers MUST decode the `bytes` per this layout, MUST require canonical words (unused bytes zero, a long form only for lengths ≥ 32), and MUST bound the declared length (reference bound: 4096 bytes).
...
3. **Verify the count, and the policy version (step 3b).**
   Prove the `itemList` length slot; require it to equal `itemCount` and
   `rows.length`. Then prove the `metaEvidenceUpdates` slot and require its
   value to be a policy version the profile accepts (§3 `policyVersions`; 0 is
   the deployment version): a value the profile does not list means the
   governor changed the policy in a way this release has not vouched for, the
   verdicts' policy basis is not one the consumer knows, and the snapshot MUST
   be rejected until a new signed profile names the version (owner decision
   2026-09-06, superseding the immutable-policy rule of 2026-08-24).
3d. **Verify the governor.** Prove slot 3 and require it to hold exactly the
   profile's `governor` (upper 12 bytes zero). A changed governor fails closed
   the same way as a changed arbitrator: the governor is who may queue every
   other change, so it is part of the trust context.
3c. **Verify the arbitrator identity.** Prove slot 0 and require it to hold exactly the profile's `arbitrator` (upper 12 bytes zero); prove slot 1, decode its declared length and form (§5), require the length to equal the pinned `arbitratorExtraData` length before any data word is consulted, then prove the data words the length occupies, decode, and require byte equality with the pin. A mismatch fails closed, naming the observed arbitrator or length; the consumer accepts a changed arbitrator, court or juror count only by installing a new signed profile (§3). Rationale: the arbitrator is who decides every later verdict; silently following a governor's switch would let a compromised or captured governor move the registry to a court of its choosing without any consumer noticing.
