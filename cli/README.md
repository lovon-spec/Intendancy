...
`curate-export`, a second binary in this crate, snapshots a Kleros **Light** Curate list from the chain itself, so that curators and integrators can rely on a file they can rerun and compare instead of an indexer or a published export. The four Kleros Scout registries on Gnosis are presets (`--registry tokens|address-tags|atq|cdn`: the address, `items` slot, creation block and label of one deployment each); any other Light list works with `--list`, an explicit `--items-slot`, and `--chain-id`/`--genesis` off Gnosis, its start block discovered unless `--from-block` is given. A preset combined with another address, chain, genesis or slot is refused rather than lending its creation block and label to a deployment they do not describe.
...
Work is bounded per item, whatever the item's shape: at most 256 blocks visited (a root under the block cap may name thousands of links to one empty block; they are one fetch and a budget error, not thousands of requests), 64 network attempts, 8 MiB downloaded and 120 s from the item's first admission, each block request's deadline being the shorter of 30 s and what is left of that budget. Blocks that hash to their CID are cached for the run (64 MiB, concurrent misses coalesced into one fetch, nothing negative), so a retry after an unavailable block replays the walk from the cache and fetches only that block; bytes that fail their hash move on to the next gateway and demote the one that served them; hash-verified bytes that are not an item file are a permanent failure, never downloaded again. At most `--concurrency` item tasks are admitted at a time and as many block requests are in flight; delayed retries wait in a queue of their own. The log scan shrinks its window only on a recognised range or result-size limit (to the limit the source names when it names one), retries transient failures at the same window with bounded backoff, and grows a shrunken window back after sustained success.
...
$B export --registry atq --anchor-block 48171346 --out items-again.json --provenance provenance-again.json   # a rerun at an earlier export's anchor reproduces its items.json byte for byte
...
# 3. The Tokens list as a Uniswap-schema token list, with a diff against the t2crtokens.eth export.
$B tokenlist --items tokens-items.json --compare https://t2crtokens.eth.limo/ --out tokens.json --diff-out diff.json --skipped-out skipped.json
```

`items.json` carries, per item, the item id, the status (name and number), the IPFS path, and the item file's `columns` and `values` verbatim, sorted by item id; statuses Registered and ClearingRequested by default, `--include-pending` adds RegistrationRequested, `--all` adds Absent with only the path its log carried. `tokenlist` reads that snapshot (or runs the export itself when given the source flags): rich addresses become chain id and EIP-55 address, decimals are validated, logos become `ipfs://` URIs, the output is deterministic, `--previous` applies token-lists versioning and `--compare` writes a diff. The list is named `Kleros Tokens Verified` unless `--name` gives another name the token-lists schema accepts (`^[\w ]+$`, 1 to 30 characters, checked before any work), and the rendered list is validated against the schema's checks (the schema is vendored under `fixtures/uniswap/`) before it is written; an item whose name or symbol the schema would reject is skipped with the reason. An item whose Address has no `eip155:` namespace (the list also holds Solana tokens under `solana:`, 410 of the 5858 registered items on 2026-09-10) is skipped with the reason recorded, and `--skipped-out` writes the skip list; no chain is guessed for an address without one. Kleros's own exporter, kleros/t2cr-to-ipfs (archived, migrated to Atlas), applied the same rule, so the comparison with the list at t2crtokens.eth, which is still regenerated, is like-for-like. `provenance.json` records the anchor and how its height was chosen (`anchorSource`), every source, the block range, the status histogram, every skipped item with its reason, the measured RPC work (`rpc`: requests, HTTP attempts, retries, bytes), the content work (`ipfs`: block requests, bytes, bad-byte responses, cache hits, coalesced requests, item retries), seconds per stage (`stageSeconds`) and the sha256 of the output. `tools/smoke-curate-export.sh` runs all of it against the live registries and asserts the invariants (every export ok, an Address Tags Query rerun at its anchor byte-identical, the token list named and validated); the comparison with t2crtokens.eth and a second Tokens export at a later anchor are informational, since different finalized anchors can legitimately differ.
