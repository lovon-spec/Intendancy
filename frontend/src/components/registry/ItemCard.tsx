    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h3 className="font-semibold text-gray-900 text-sm truncate">
          {item.fields?.name || "Malformed descriptor"}
        </h3>
      ...
      {item.descriptorError && (
        <p className="mt-2 text-xs text-red-700 line-clamp-2">
          Policy-invalid descriptor: {item.descriptorError}
        </p>
      )}
      {item.requestDisputed && item.displayStatus === "flagged" && (
        <p className="mt-2 text-xs font-medium text-red-800">Removal request is disputed</p>
      )}
    </Link>
  );
}
